...
function App() {
  ...
  const page = path === '/' ? <Landing /> : path === '/dashboard' ? <Dashboard /> : <NotFound />
  return <AppErrorBoundary>{page}</AppErrorBoundary>
}

function NotFound() {
  return <main className="error-screen"><Shield size={34} /><div className="section-kicker">404 / SIGNAL LOST</div><h1>This route is outside the shield.</h1><p>The page does not exist. Return to the public experience or open the command center.</p><div className="error-actions"><a className="button button-primary" href="/">Return home <ArrowRight size={16} /></a><a className="button" href="/dashboard">Open dashboard</a></div></main>
}
